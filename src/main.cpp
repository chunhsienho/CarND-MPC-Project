    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          double angle_steer=j[1]["steering_angle"];
          double pedal_throttle=j[1]["throttle"];
          
          const double Lf = 2.67;

          //control delay simulation
          // by using the latency we could apply the latency
          double latency=0.1;

          px= px +v*cos(psi)*latency;
          py=py+v*sin(psi)*latency;
          //steering angle woule be opposit direction for simulation
          ...
          */
          ...
          //Transform the ptsx ptsy from global into car coordinate
          ...
          //the epse= psi0-atan(coeffs[1]+2*x0*coeffs[1]+3*coeffs[2]*pw(x0,2))
          ...
          msgJson["steering_angle"] = -steer_value;//opposite direction
          ...
          double predict_y=0;
          for(size_t i=2;i<solution.size();i=i+2){
